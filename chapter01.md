...
inotifyはinode[^inode]ベースの通知であり、ディレクトリ単位での再帰的な自動監視は行いません。再帰的に監視したい場合は、利用する側がディレクトリツリーを辿ってそれぞれを監視対象に追加する必要があります。
...
[^gatekeeping]: 作に介入して許可・拒否等の処理を行うための機能。
...
BSD自体にはファイルシステム監視機能はありませんでした。しかし、子孫であるFreeBSDには、4.1からkqueue[^kqueue]という機能が追加されました。
