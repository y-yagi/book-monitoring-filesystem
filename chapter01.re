...
さて、先に説明した通り、ポーリングではファイル変更監視を正確に行うのは難しいです。しかし、昨今のOSは、OS側でファイル変更監視の為の仕組みが提供されています。APIは、一般的に、監視したファイルやディレクトリをAPIに指定->指定したファイルやディレクトリで変更があったら変更内容を取得、という事が出来るようになっています。
...
Linuxにはじめて導入されたファイルシステムイベント監視の仕組みは、dnotifyという機能で、Linux 2.4.0で導入されました。dnotifyはディレクトリ自体、もしくはその内部のファイルに変更がかかったときに、ユーザアプリケーションにその変更を通知する事を目的として提供されていました@<fn>{dnotify}。変更の通知はシグナル送信により行われます。
//footnote[dnotify][dnotifyの先頭の"d"は、"directory"の"d"のらしいです。]
