...
なお、上記のような説明があるものの、少しづつ方針が変わっているようです。例えば、以前は再帰的な監視処理がLinuxでは動作しない、という問題がありましたが、現在はこの問題は解消されており@<fn>{recursive_linux}、影響が大きい差異についてはAPIでOSの差異の吸収を行うようになっているようです。
