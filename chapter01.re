...
inotifyはinode@<fn>{3}ベースの通知であり、ディレクトリ単位での再帰的な自動監視は行いません。再帰的に監視したい場合は、利用する側がディレクトリツリーを辿ってそれぞれを監視対象に追加する必要があります。
...
BSD自体にはファイルシステム監視機能はありませんでした。しかし、子孫であるFreeBSDには、4.1からkqueue@<fn>{6}という機能が追加されました。
...
続いてmacOSについて見ていきましょう。macOSが使用しているDarwinというOSがBSDをベースにしており、他のBSD同様に、kqueueがサポートされています。それに加えて、macOSでは、FSEvents@<fn>{7}という機能も使えるようになっています。次のセクションで詳細を見ていきましょう。
...
ReadDirectoryChangesW@<fn>{8}は、Windows NT 4.0で導入されたより高機能なディレクトリ変更監視APIです。
...
//footnote[5][作に介入して許可・拒否等の処理を行うための機能。]

//footnote[6][Kernel Queuesの略。]

//footnote[7][File System Eventsの略。]

//footnote[8][末尾の"W"までが関数名です。Win32では、文字列の符号化方法がANSI互換文字列の場合末尾が"A"、ワイド文字列の場合"W"が使われるようになっています。FindFirstChangeNotificationについては、「FindFirstChangeNotificationA」と「FindFirstChangeNotificationW」の両方が存在しています。]
